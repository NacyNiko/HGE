        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                if self.model_name in ['TNTComplEx', 'TComplEx']:
                    ...
                    (a, b), time_ent = self.get_queries(queries)
                    scores = a @ rhs + torch.sum(b * time_ent, dim=1).unsqueeze(1) + b @ rhs + torch.sum(a * time_ent,
                                                                                                         dim=1).unsqueeze(
                        1)
                else:
                    q, tmp = self.get_queries(queries)
                    scores = q[0] @ rhs + torch.sum(q[1] * tmp, dim=1).unsqueeze(1)
                #scores = q @ rhs
                # scores = q
                # set filtered and true scores to -1e6 to be ignored
...
class TLT_KGE_Quaternion(TKBCModel):
    def __init__(
            ...
            init_size: float = 1e-3, cycle=365, use_attention=0, att_weight=0.5, num_form='split'
    ):
...
class HGE_TLT_KGE(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            init_size: float = 1e-3, cycle=365, attention=0, num_form='complex', use_reverse=False
    ):
        super(HGE_TLT_KGE, self).__init__()
        self.cycle = cycle
        self.model_name = "HGE_TLT_KGE"
        self.sizes = sizes
        self.rank = rank
        self.pi = 3.14159265358979323846
        self.linear = nn.Linear(rank, rank//2)
        ...
        self.context_vec = nn.Embedding(self.sizes[1], self.rank*2)
        self.context_vec.weight.data = init_size * torch.randn(self.sizes[1], self.rank*2)
        self.rel_vec = nn.Embedding(self.sizes[1], self.rank*2)
        self.rel_vec.weight.data = init_size * torch.randn(self.sizes[1], self.rank*2)
        self.act = nn.Softmax(dim=1)
        self.embeddings = nn.ModuleList([
            nn.Embedding(sizes[0], 2*rank, sparse=True),
            nn.Embedding(sizes[1], 2*rank, sparse=True),
        ...
        self.rel_att_weight=init_size
        self.geo_att_weight=init_size
        if rank % 2 != 0:
            raise "rank need to be devided by 2.."
        for i in range(len(self.embeddings)):
            torch.nn.init.xavier_uniform_(self.embeddings[i].weight.data)
    ...
    def cal_num_form(self,x,y,input,num='split'):
        if num=='split':
            ...
            if num =='rel':
                query = self.rel_vec(input[:, 1])
            else:
                query = y
            att_l, att_r = [self.cal_att_num_form(split[i], dual[i], complex[i], query[i]) for i in [0,1]]
            #print(att_r)
            self.geo_att_weight = att_l[1], att_r[1]
            return att_l[0], att_r[0]

    def cal_att_num_form(self, x, y, z, query):

        x, y, z, query = [i.view((-1, 1, self.rank*2)) for i in [x,y,z, query]]
        ...
        geo_att_weight = torch.sum(query * cands /np.sqrt(self.rank*2), dim=-1, keepdim=True)
        geo_att_weight = self.act(geo_att_weight)
        #print(self.att_weight[0][:5])
        att_lhs = torch.sum(geo_att_weight * cands, dim=1)
        return att_lhs, geo_att_weight

    def cal_rel(self,x, rnt, rwt):

        if self.use_attention==1:
            rwt = rwt.view((-1, 1, self.rank*2))
            rnt = rnt.view((-1, 1, self.rank*2))
            cands = torch.cat([rwt, rnt], dim=1)
            context_vec = self.context_vec(x[:, 1]).view((-1, 1, self.rank*2))
            #context_vec = self.context_vec(x[:, 0]).view((-1, 1, self.rank))
            self.rel_att_weight = torch.sum(context_vec * cands /np.sqrt(self.rank*2), dim=-1, keepdim=True)
            self.rel_att_weight = self.act(self.rel_att_weight)
            #print(self.att_weight[0][:5])
            full_rel = torch.sum(self.rel_att_weight * cands, dim=1)
        else:
            full_rel = rwt + rnt
        return full_rel
    ...
    def score(self, x):
        ...
        comp_time = self.embeddings[5](x[:, 3])
        rel_ = rel*comp_time
        rel_ = self.cal_rel(x, rel, rel_)
        real_part , img_part = self.cal_num_form((lhs, time_ent), (rel_, time),x, self.num_form)
    ...
    def get_queries(self, queries: torch.Tensor):
        ...
        comp_time = self.embeddings[5](queries[:, 3])
        rel_ = rel*comp_time
        rel_ = self.cal_rel(queries, rel, rel_)
        real_part, img_part = self.cal_num_form((lhs, time_ent), (rel_, time),queries, self.num_form)
        if self.use_reverse:
            lhs_rel_1 = real_part+ img_part
            lhs_rel_2 = -real_part + img_part
        else:
